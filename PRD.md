# System-Bar PRD — macOS 菜单栏实时系统指标

## 1. 背景

在 Apple Silicon（M1）Mac 上，以**最小资源开销**在菜单栏实时显示系统指标，
各项指标可在菜单中独立开关配置。

## 2. 目标

- 菜单栏**直接显示数字**，实时刷新（≤2s），如 `74° 25% 62% 4.2W`
- 免 root、免装额外驱动
- 支持开关配置，持久化记忆
- 空闲开销尽可能小：CPU ≈ 0%、内存 < 40MB、几乎不影响电量
- 下拉菜单可查看全部温度传感器并退出

### 2.1 支持的指标（v2）

| 指标 | 单位 | 定义 | 读取途径 |
|---|---|---|---|
| CPU 温度 | °C | P/E 核传感器最大值，回退 tdie | IOHID（已验证） |
| 电池温度 | °C | `gas gauge battery` 传感器 | IOHID（已验证） |
| CPU 占用 | % | 总体 CPU 使用率 | `host_processor_info`（已验证） |
| 内存占用 | % | used / total 内存 | `host_statistics64`（已验证） |
| GPU 占用 | % | GPU 利用率 | AGXAccelerator（已验证） |
| 实时功耗 | W | macmon 口径：CPU+GPU+ANE | IOReport（已验证） |
| 上传/下载速度 | KB/s·MB/s | 活动网卡字节计数差值 | sysctl iflist2（已验证） |
| 电池健康 | % + 循环次数 | 当前容量/设计容量、循环次数 | AppleSmartBattery IORegistry（已验证） |
| 电池剩余时间 | 时:分 | 当前容量 ÷ 放电电流（放电时有效，插电 `--`） | AppleSmartBattery IORegistry（已验证） |
| 内存压力 | 三级 | 正常/警告/严重（🟢🟡🔴） | sysctl `kern.memorystatus_vm_pressure_level`（已验证） |
| 磁盘读写/剩余 | KB/s·MB/s + % | 磁盘字节计数差值 + 剩余百分比（statvfs 剩余/总量） | IOBlockStorageDriver 统计 + statvfs（已验证） |

## 3. 非目标（明确不做）

- 不做历史曲线/图表
- 不做复杂配置界面
- 不支持 Intel Mac（如后续需要可降级为 `osx-cpu-temp` 读取）

## 4. 技术方案

| 项 | 选择 | 说明 |
|---|---|---|
| 系统要求 | macOS 13+ | 使用原生 AppKit `NSStatusItem` + `NSPopover`（曾用 `MenuBarExtra`，因 macOS 13 状态栏项消失/面板状态错乱弃用） |
| 芯片 | Apple Silicon 专用 | M1 实测；其他 M 系列待验证 |
| 温度来源 | HID SMC 事件接口 | `AppleARMPMUTempSensor` 节点，免 root |
| CPU 温度定义 | `max(pACC ∪ eACC)`，回退 `tdie` | P 核/E 核传感器最大值 |
| 配置持久化 | `UserDefaults` | 开关状态重启后保留 |
| 架构 | C helper 常驻 + SwiftUI App | helper 周期采样推送，App 只管 UI |

### 架构图

```
NSStatusItem + NSPopover App（菜单栏 "74° 25% M75% 6.1W ↓12K ↑955B"，下拉含开关/进程）
        │  NSPipe 读取 stdout，按行解析
        ▼
C helper (smctemp)  常驻进程
  ├─ IOHID 温度读取（事件驱动）
  ├─ host_processor_info  CPU 占用
  ├─ host_statistics64    内存占用
  ├─ AGXAccelerator       GPU 占用
  ├─ IOReport             实时功耗
  └─ 每 2s 输出一行 "key=value;..."
```

- App 启动时拉起 helper，helper 异常退出则自动重启
- 指标读取为低频采样（≤2s），空闲时 helper 几乎零 CPU

## 5. 交互与视觉

- 菜单栏：显示**已开启**的指标，如 `74° 25% 62% 4.2W`（CPU 温度、CPU%、内存%、功耗）
- **电池温度老化提醒符号**（菜单栏模板渲染下文字颜色不可靠，采用符号方案）：
  - `≤ 40°C`：正常，无符号
  - `> 40°C`：特别提醒（高温）→ `🔥`
- 下拉面板（NSStatusItem + NSPopover transient 实现，替代 MenuBarExtra——macOS 13 上 MenuBarExtra 状态栏项会消失/面板状态错乱）：
  - **关闭行为**：点击面板外任意处关闭；<kbd>Esc</kbd> 关闭；✕ 杀进程不关闭（可连续杀多个）；再次点击图标切换
    - macOS 15 起必须显式 `NSApp.activate(ignoringOtherApps:)` + `makeKey()` 才能恢复"点外部自动关闭"（见 §11 风险表、F9）
    - 关闭判定抽为 `PopoverPolicy`（纯计算可单测），并配 local/global 鼠标事件 + `didResignActive` 兜底，防止系统行为再次变化回归
  - **指标开关组**：CPU 温度 / 电池温度 / CPU 占用 / 内存占用 / GPU 占用 / 实时功耗 / 上传下载 / 磁盘 / 内存压力（Toggle，即时生效并持久化）
  - **电池健康行**：`电池健康 92% · 120 次循环`（数据可用时显示，无需开关）
  - **电池剩余时间行**：`电池剩余 1h 11m`（估算 = 当前容量 ÷ 放电电流，仅电池放电时有效；充电/插电显示 `--`）
  - **磁盘剩余行**：`磁盘剩余 18% · 19G / 108G`（与电池/今日最高同区显示，无需开关）
  - **磁盘清理**：清空废纸篓 + 用户缓存（`~/Library/Caches`），按钮显示可回收量，**需二次点击确认**，完成后提示本次释放量
  - **桌面模拟时钟**：面板「桌面时钟」按钮打开独立浮窗（黑底模拟钟面 + 橙色秒针，每秒刷新；下方数字行日期+星期+时间）。浮窗可拖动/缩放、常驻桌面、跨空间可见，开关状态持久化
  - 「退出」
- 无 Dock 图标（`LSUIElement`）
- **防跳动**：每段使用**定宽右对齐 + 等宽数字**，单段字符数不随数值位数变化（温度/百分比 3 位、功耗 6 字符含 W、网速/磁盘 4 字符右对齐）。功耗用 `Format.powerText` 统一右对齐到 6 字符；网速 `speedText` 保证 ≤4 字符（1000–1023 B 进位为 `1.0K`，不产生 5 字符）。数值位数变化时菜单栏布局不变
- **每日最高温度记录**：按天记录 CPU/电池最高温度到 `~/Library/Application Support/System-Bar/daily-temps.log`（每行 `2026-08-12 cpu=89.2 bat=44.1`），菜单显示今日最高，长期监控老化趋势
- **进程管理**：下拉菜单按内存占用列出**前 7 个**进程（名称 / **内存占总量百分比**，不显示其他指标），行首 `✕` 标识，整行可点击一键强制结束
  - **系统进程（root 属主）需连点两次 `✕` 确认**，普通用户进程一次即杀；杀失败显示提示
  - 数据源：libproc（`proc_listallpids` / `proc_pidinfo`），5s 刷新
  - 安全：排除自身与 helper；仅限同用户进程（系统进程无权限自然失败并提示）
- **内存压力并入内存占用**：内存占用开启且内存压力开启时，显示为 `M75%🟢`（圆点并入）；内存占用关闭但内存压力开启时，单独显示 `🟢`；两者都关则不显示（F8）
- 注：不展示原始温度传感器列表（名称晦涩无实用价值）

### 5.1 提醒阈值依据

锂电池老化三要素：高温 + 高电量 + 充电电流。40°C 以上进入明显加速老化区间，
触发特别提醒符号（🔥）。阈值可由用户调整。

## 6. 验收指标

| 指标 | 目标 |
|---|---|
| 菜单栏刷新延迟 | ≤ 2s |
| 空闲 CPU 占用（App + helper 合计） | < 3%（单核） |
| 常驻内存 | < 40MB |
| 读数合理性 | 温度 20–110°C、占用 0–100%、功耗 0–100W，与系统/专业工具趋势一致 |
| 开关即时生效且重启保留 | 是 |
| 无 root / 无额外安装 | 是 |
| 菜单栏无跳动 | 数字位数变化时布局宽度不变 |

## 7. 测试策略

1. **helper 单元测试**：反复运行，断言输出格式合法、各指标在合理区间、无崩溃
2. **阈值逻辑测试**：电池温度分级边界值（40°C）断言
3. **格式化测试**：`Format.swift` 的 pad/speedText/powerText/freeText/timeText 边界值 + 等宽断言（含功耗 6 字符定宽、网速 ≤4 字符跳宽防护）
4. **面板关闭策略测试**：`PopoverPolicy.swift` 点击判定（面板内 / 状态栏按钮 / 面板外 / 坐标缺失）+ Esc + 坐标翻转边界（25 项）
5. **集成测试**：App 启动后收到 helper 全量数据，开关切换即时反映
6. **手动验收**：菜单栏文字、开关、提醒符号、防跳动（数值位数变化布局不变）、
   面板关闭（点击外部 / <kbd>Esc</kbd> / 切到别的 App / 再次点击图标）、退出

## 8. 里程碑

1. ✅ v1：HID 温度读取 + 菜单栏显示 + 打包（已完成）
2. ✅ 验证 GPU 占用、功耗免 root 读取方案（已完成）
3. ✅ 验证 CPU/内存占用读取方案（已完成）
4. ✅ helper 扩展输出多指标并跑通测试（已完成）
5. ✅ App 菜单栏多指标 + 开关配置 + 打包验证（已完成）
6. ✅ F1 电池健康 / F2 磁盘读写+剩余空间（已完成）
7. ✅ F3 GitHub Actions CI + main 分支保护（已完成：ci.yml 含 build + format + dailylog + procmon + smctemp(arm)）
8. ✅ F4 每日最高温度记录（老化趋势，已完成）
9. ✅ F5 进程管理：前 7 个按内存列出 + 整行按钮 + 系统进程两次确认（已完成）
10. ✅ F6 下拉面板交互：NSStatusItem+NSPopover、面板外点击关闭、✕ 不关面板可连杀（已完成）
11. ✅ F7 电池剩余时间估算（容量÷放电电流，插电显示 `--`）（已完成）
12. ✅ F8 内存压力三级显示，并入内存占用段（`M75%🟢`）（已完成，含防跳动对齐优化）
13. ✅ F9 修复 macOS 15 下面板点外部/切走不收起：`NSApp.activate` + `makeKey` 恢复
    `.transient` 关闭链路，`PopoverPolicy` 事件兜底 + Esc 关闭（已完成，PRD + 测试 25/25）
14. ⏳ 桌面模拟时钟：**文档已描述、代码未落地**（目前仅备好 `Format.clockText`）

> 说明：F5–F9 的代码均已落地并配套测试；本文档已在新版中追平（指标表、交互、防跳动口径）。

### Todo / 后续候选（超出当前 PRD 范围，供产品规划参考）
- 电池健康趋势报告（基于 daily-temps.log 的长期老化汇总）
- 指标语义化输出（把裸数字转成"健康结论"）
- 下拉面板分组/折叠，避免开关过多溢出
- 更多机型（M2–M5）实测与传感器命名回退覆盖

## 9. 工程化（CI）

- **GitHub Actions CI**：push / PR 自动执行
  - 格式化测试（`test_format.sh`，全平台）
  - helper 测试（`test_smctemp.sh`，仅 arm64 runner）
  - 完整构建（`build.sh`）
- **main 分支保护**：CI 通过为合并门槛

## 10. GPU 占用与功耗的技术途径

两个指标在 M1 免 root 读取存在不确定性，先做验证：

| 指标 | 候选方案 | 说明 |
|---|---|---|
| GPU 占用 | 参考 mxmon `collect/gpu.rs` | mxmon 声称免 sudo 读 GPU 利用率，验证其实际数据源 |
| 实时功耗 | 候选 A：SMC `PSTR` 键 | M1 上 SMC IOKit 读取受限，待验证是否可行 |
| | 候选 B：IOReport 能耗模型 | 类似 MxPowerGadget/macpow，从 CPU 性能状态估算整机功耗 |

验证失败时：对应开关置灰/显示 `--`，其余指标不受影响。

## 11. 风险与对策

| 风险 | 对策 |
|---|---|
| **macOS 15 起 accessory App 的 NSPopover 不再自动激活 App**，`.transient` 的"点击外部自动关闭"失效（实测：show 后 `NSApp.isActive=false`、面板窗口 `isKeyWindow=false`，切到别的 App 仍 `isShown=true`） | 显示前 `NSApp.activate(ignoringOtherApps: true)` + 显示后 `makeKey()` 恢复原生链路（实测切走后 `isShown=false`）；另配 local/global 鼠标事件监听 + `didResignActive` 兜底，判定抽为 `PopoverPolicy`（25 项单测），防止系统行为再次变化回归 |
| 菜单栏模板渲染导致文字颜色不生效 | 回退方案：数字前加 ⚠（偏高）/ 🔥（高风险）符号，符号渲染 100% 生效 |
| HID 事件客户端类型/API 随 macOS 版本变化 | 尽量用公开稳定的调用；helper 单文件便于快速 patch |
| 传感器命名因机型而异（tdie 缺失等） | 取核传感器最大值，缺失时回退到任一可用温度传感器 |
| GPU 占用 / 功耗在部分机型不可读 | 开关灰显，菜单栏显示 `--` |
| 充电上限（SMC 写）| ❌ 已实测不可行：CH0B/CH0C 数据读取被拒(133)、BCLM 键不存在(132)，即使 root。替代：macOS 15 自带 80% 限制或 AlDente |
| Intel 支持 | 明确不支持（PRD 范围），后续可用 `osx-cpu-temp` 分支 |
